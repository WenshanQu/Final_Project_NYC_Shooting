...
history_raw = read_csv("./data/historic.csv") %>% janitor::clean_names()
year_2021_raw = read_csv("./data/year_to_date.csv") %>% janitor::clean_names()
full_raw_label_df = 
  bind_rows(history_raw, year_2021_raw) %>% 
  mutate(
    text_label = str_c("Occur time: ", occur_date, " ", occur_time, " Boro: ", boro)
  )
...
risk_plot = 
  ...
  addCircleMarkers(
    ...
    ~ latitude,
    radius = 5,
    label = ~ full_raw_label_df$text_label
  )
...
```{r}
test_df = read_csv("./comp_data/Full NYC Shooting Data (2006-2021).csv")
